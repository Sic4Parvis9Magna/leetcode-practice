 ...
 * STATUS: COMPLETED
 * 
 * Solution: O(n) in time and O(1) in memory(as we asked to create duplicated
 * list and do not store anything extra)
 * 
 */
...
class Solution {
    public Node copyRandomList(Node head) {
        duplicateNodes(head);
        duplicateRandomLinks(head);

        return collectCopyList(head);
    }
    ...
    private void duplicateRandomLinks(Node head) {
        Node currentNode = head;
        while (currentNode != null) {
    ...
    private Node collectCopyList(Node head) {
        if (head == null) {
            return null;
        }
        ...
        Node currentNode = resultHead.next;
        while (currentNode != null) {
            Node copyNode = currentNode.next;

            currentNode.next = copyNode.next;
            resultTail.next = copyNode;
            resultTail = copyNode;
