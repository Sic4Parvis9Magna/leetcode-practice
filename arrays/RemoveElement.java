...
public class RemoveElement {
    public static void main(String[] args) {
        ...
        int[] arr = {3,2,2,3};
        int val = 3;
        int result = sol.removeElement(arr, val);
...
class RemoveElementSolution {
    public int removeElement(int[] nums, int val) {
        ...
        for (int index = 0; index < nums.length; index++) {
            while(nums[index] == val && index < nums.length - matches) {
                shiftOneStepLeft(nums, index);
                matches++;
            }
        }
    ...
    public void shiftOneStepLeft(int[] array, int index) {
